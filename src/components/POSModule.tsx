...
export const POSModule = ({ currentCompany, addPendingOrder }: POSModuleProps) => {
  ...
  // Carregamento leve: exibe 15 inicialmente, expansível sob demanda
  ...
  // Carregamento ultraleve sob demanda: renderiza apenas os primeiros N itens (15 padrão)
  const visibleSalesHistory = useMemo(() => {
    if (historySearch.trim().length >= 2) {
      return filteredSalesHistory.slice(0, Math.max(historyDisplayLimit, 50));
    }
    return filteredSalesHistory.slice(0, historyDisplayLimit);
  }, [filteredSalesHistory, historyDisplayLimit, historySearch]);
        ...
        <div className="flex items-center gap-1.5 overflow-x-auto custom-scrollbar">
          {[
            ...
            <button
              ...
              className={cn(
                "flex items-center gap-2 px-3 py-2 rounded-xl text-xs font-bold uppercase tracking-wider transition-all whitespace-nowrap cursor-pointer active:scale-95",
                activeTab === tab.id
      ...
      {activeTab === 'historico' && (
        <div className="flex-1 min-h-0 p-4 sm:p-6 flex flex-col overflow-hidden">
          ...
          <div className="flex-1 min-h-0 bg-white/5 border border-white/10 rounded-2xl overflow-y-auto custom-scrollbar">
            ...
            {/* Controles de Carregamento Sob Demanda */}
            {filteredSalesHistory.length > 0 && (
              <div className="p-3 bg-slate-950/80 border-t border-white/10 flex flex-wrap items-center justify-between gap-2 text-xs">
                <span className="text-white/50 text-[11px]">
                  Mostrando <strong className="text-white">{visibleSalesHistory.length}</strong> de <strong className="text-white">{filteredSalesHistory.length}</strong> vendas
                </span>
                <div className="flex items-center gap-2">
                  {visibleSalesHistory.length < filteredSalesHistory.length ? (
                    <>
                      <button
                        type="button"
                        onClick={() => setHistoryDisplayLimit(prev => prev + 20)}
                        className="px-3 py-1.5 rounded-lg bg-primary-600 hover:bg-primary-500 text-white font-bold text-[11px] transition-colors"
                      >
                        + Carregar mais 20
                      </button>
                      <button
                        type="button"
                        onClick={() => setHistoryDisplayLimit(filteredSalesHistory.length)}
                        className="px-3 py-1.5 rounded-lg bg-slate-800 hover:bg-slate-700 text-white/80 hover:text-white text-[11px] transition-colors border border-white/10"
                      >
                        Carregar Todas ({filteredSalesHistory.length})
                      </button>
                    </>
                  ) : filteredSalesHistory.length > 15 ? (
                    <button
                      type="button"
                      onClick={() => setHistoryDisplayLimit(15)}
                      className="px-2.5 py-1 rounded-lg bg-slate-800/80 hover:bg-slate-700 text-white/60 hover:text-white text-[10px] transition-colors"
                    >
                      Voltar ao Modo Leve (15)
                    </button>
                  ) : null}
                </div>
              </div>
