...
export const MessagesSidebarPopup: React.FC<MessagesSidebarPopupProps> = ({
  ...
  return (
    <>
      ...
      {/* Popup */}
      <GlassCard className="fixed left-0 top-0 bottom-0 w-96 bg-white/5 border-white/10 rounded-none border-l-0 border-y-0 border-r p-0 overflow-hidden flex flex-col flex-shrink-0 z-50 animate-in slide-in-from-left duration-300">
        {/* Header */}
        ...
        {unrepliedCount > 0 && viewFilter !== 'unreplied' && (
          <div className="mx-6 mt-4 p-3 bg-rose-500/10 border border-rose-500/20 rounded-2xl flex items-center justify-between animate-pulse flex-shrink-0">
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar">
          {filteredLeads.map(l => {
            ...
            return (
              <div 
                key={l.id} 
                onClick={() => handleSelectLead({ ...l, name: l.fullName } as Lead)}
                className={cn(
                  "p-3 border-b border-white/5 cursor-pointer transition-all group relative hover:bg-white/5"
                )}
              >
                <div className="flex justify-between items-start mb-1 gap-2">
                  <div className="flex items-center gap-2 truncate">
                    <p className={cn("font-bold transition-colors truncate text-sm text-white group-hover:text-primary-300")}>{l.fullName}</p>
                    {waitingSinceDate && (
